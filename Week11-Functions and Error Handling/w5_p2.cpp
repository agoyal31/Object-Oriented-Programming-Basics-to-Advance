...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		do
		{
			std::string strBookRecord;

			std::getline(file, strBookRecord);
			...
			if (strBookRecord[0] != '#' && strBookRecord.length() > 1 && cnt <= 3) {
				Book tempBook(strBookRecord);
				//library[cnt++] = tempBook;
				library += tempBook;
				cnt++;
		...
		while (!file.eof()) {
			...
			//   in error mode
				...
				// Check if this is a commented line.
				//   In the input file, commented lines start with '#'
			if (strBookRecord[0] != '#' && strBookRecord.length() > 1) {
				Book tempBook(strBookRecord);
				library += tempBook;
	...
	if (argc > 2) {
		...
		do
		{
			...
			//   in error mode
				...
				// Check if this is a commented line.
				//   In the input file, commented lines start with '#'
			if (strMovie[0] != '#' && strMovie.length() > 1) {
				Movie tempMovie(strMovie);
