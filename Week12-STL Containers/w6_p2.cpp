...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* tempVehicle) {
			if (tempVehicle->topSpeed() > 300) {
				return true;
			}
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* tempVehicle) {
			if (tempVehicle->condition() == "b") {
				return true;
			}
